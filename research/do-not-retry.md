...
- **September 4 fine-grained requalification (campaign `sps-requal-20260904-now`): closed at calibration, no serving candidate.** The reopen condition above was met by SGLang PR #37815 (fine-grained M-bin fitting, pinned head `085a5e2a`). Two independent additive-SPS profiles (P0, P1; 120 cells each, bs 1-8 x frac 0.2-1.0 x 3 repeats, M=2-48) passed every correctness gate. Fitting on P0 and scoring on held-out P1 gave MAE `0.1372` ms at width 64, `0.1440` ms at width 6, and `0.1460` ms at width 1, against a within-run repeat spread of `0.2955` ms. Finer widths were worse on MAE, RMSE, max error, and bias; the predeclared gate required a 50% MAE improvement. Root cause is physical, not a fitting defect: on TP1/GB300 the DSpark verify step is flat in M across 2-48 (bs=8 moves 16.3 -> 16.2 ms from M=16 to M=48), so the per-token theta term the table exists to model is below collection noise. `CONTROL=STOP` was set before any S-card was instantiated; R0/C0/C1/C2/R1 were never released.
- **Reopen condition (revised):** only if a runtime change makes verify cost measurably M-dependent on this hardware (a different verify kernel path, TP>1, or a different drafter), demonstrated first by a two-profile held-out fit that clears the 50% gate. Do not re-run the additive SPS bracket on this pinned image/TP1 surface. Evidence: `inner-loop/campaigns/sps-requal-20260904-now/artifacts/fit-r1/fit-manifest.json` (sha256 `d324a40fe12d9954d4c3c9230c32aa918d89ba0d236aa5d680319b57bc0638b0`).
